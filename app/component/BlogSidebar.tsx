    ...
    <aside className="space-y-8">
      ...
      <div className="bg-white rounded-xl p-6 shadow-md">
        ...
        <div className="space-y-4">
          {recentPosts.slice(0, 4).map((post) => (
            <Link key={post.id} href={`/blog/${post.slug}/`} className="group flex gap-4 p-2 -mx-2 rounded-xl hover:bg-neutral-50 transition-all duration-200">
      ...
      <div className="bg-white rounded-xl p-6 shadow-md">
        ...
        <ul className="space-y-4">
          {blogCategories.filter(cat => cat !== "All").map((category) => (
            <li key={category}>
              <Link href={`/blog/?category=${encodeURIComponent(category)}`} className="group flex items-center justify-between px-4 py-2.5 rounded-xl hover:bg-indigo-50 transition-all duration-200">
        ...
        <div className="bg-gradient-to-br from-indigo-50 to-blue-50 rounded-xl p-6 shadow-md">
          ...
          <div className="space-y-4">
            {relatedPosts.map((post) => (
              <Link key={post.id} href={`/blog/${post.slug}/`} className="group block p-4 bg-white rounded-xl shadow-sm hover:shadow-md transition-all duration-200 hover:-translate-y-0.5">
      ...
      <div className="bg-indigo-300 rounded-xl p-6 shadow-lg text-white overflow-hidden relative">
        ...
        <div className="relative z-10 space-y-4">
          ...
          <p>Transform your scheduling with GetSetTime. Get started today.</p>
